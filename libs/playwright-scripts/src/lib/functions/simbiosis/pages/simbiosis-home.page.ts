...
export class SimbiosisHomePage {
  ...
  async goToDetails() {
    ...
    await licenseLink.waitFor({ state: 'visible', timeout: 1000 });
    await licenseLink.click();
    await this.page.waitForSelector('div.modal.show', { timeout: 1000 });
    await this.page.waitForSelector('div.modal.show >> text=Loading...', { state: 'hidden', timeout: 1500 });
  }
